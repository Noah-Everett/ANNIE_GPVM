...
# in case we copied them in along with this script export PATH=${PATH}:.

export VERSION=2016-06-28
echo " "
echo "$0 $@"
...
export GDMLLOC=/annie/app/users/neverett/geometry/
export UNITS="-L cm -D g_cm3"

export GENLIST="" # equivalent to "Default"

# genie version for setup as of 2022-04-04
export GVERS="v3_00_04"
export GQUAL="e9:debug:r5"
export XSECQUAL="defaultplusccmec"
...
#  # updated selection as of 2022-04-04
#  
#  if [ -z "$GVERS"    ]; then export GVERS="v3_00_06_extgen"            ; fi
#  if [ -z "$GQAUL"    ]; then export GQUAL="e9:debug:r5"         ; fi
#  if [ -z "$XSECQUAL" ]; then export XSECQUAL="defaultplusccmec" ; fi
#
#  source /grid/fermiapp/products/genie/local/setup
#  echo "!-- Just sourced setup --!"
...
process_args() {
  ...
  TEMP=`getopt -n $0 -s bash -a \
     --longoptions="help outdir: run: nevt: exposure: pots: seed: gdml: topvol: mesthre: maxpl: with-mec genlist: xsec-qual: xsecfile: fid: expand: rockbox:: rock helpfid" \
     -o hvo:r:n:e:s:z:g:t:f:S:F:E:R:-: -- "$@" `
  eval set -- "${TEMP}"
  unset TEMP
  while [ $# -gt 0 ]; do
    ...
    case "$1" in
      ...
      -o                ) export OUTDIR="$2"     shift  ;;
      -z                ) export ZMIN="$2"       shift  ;;
      -s | --seed       ) export SEED="$2";      shift  ;;
      -g | --gdml       ) export GDML="$2";      shift  ;;
      -t | --topvol     ) export TOPVOL="$2";    shift  ;;
      -f                ) export FLUXFILE="$2";  shift  ;;
      --mesthre         ) export MESTHRE="$2";   shift  ;;
      --maxpl           ) export MAXPL="$2";     shift  ;;
      --with-mec        ) export GENLIST="DefaultPlusCCMEC" ;;
      --genlist         ) export GENLIST="$2";   shift  ;;
      --xsec-qual       ) export XSECQUAL="$2";  shift  ;;
      --xsecfile        ) export XSECFILE="$2";  shift  ;;
      -S                ) export NRAYS="$2";     shift  ;;
...
#    export NRAYS="${NRAYS}"
  ...
  if [ -z "$FLUXFILE" ]; then
    export FLUXFILE="/annie/data/flux/bnb/bnb_annie_0000.root"
  fi
  export FLUX=${FLUXFILE},${FLXPSET}
  ...
  export GENIEXSEC=/annie/app/users/neverett/xsec/gxspl-FNAL${XSECFILE}.xml

  if [-z "$MESTHRE" ]; then
    echo "I need something here becuase I dont know what the opposite of -z is"
  else
    export MESTHRE=/annie/app/users/neverett/config/${MESTHRE}.xml
  fi

  export MAXPL="-m /annie/app/users/neverett/geometry/${MAXPL}.maxpl.xml"
}
##########################################################################
helpfid() {
cat >&2 <<EOF
...
#export OUTDIR="/annie/app/users/neverett/runs/run_$RUNBASE"
#mkdir $OUTDIR
#cd $OUTDIR
...
if [ -z "$EXE" ]; then
  echo "$b0: failed to find ${EXETRYLIST}"
  exit 127
fi
echo "using $EXE"
...
# export GXMLPATH=${GXMLPATH}:/annie/app/users/neverett/flux/GNuMIFlux.xml
export GXMLPATH=/annie/app/users/neverett/flux:${GXMLPATH}
# echo $GXMLPATH

# sadly utils::app_init::MesgThresholds() doesn't support multiple files
# so we'll need to construct one ...
export MYMESG="/annie/app/users/neverett/other/MyLog4CPP.xml"
...
#  echo "ERROR: could not find flux files in local_flux directory"
#else
#  # make a local copy of 10 files based on a simple algorithm
#  #   assume 5000 files (i.e. 500 groups of 10)
#  FLXDIR="flux" # changed flux dir from local_flux to flux
#  let flxgrp=${RUN}%500
#  flxgrp3=`printf "%03d" $flxgrp`
#  if [ ! -d $FLXDIR ]; then mkdir -p $FLXDIR; fi
#  for i in `seq 0 9`
#  do
#     flxbase=gsimple_beammc_annie_${flxgrp3}${i}.root
...
#  export FLUX="$FLXDIR/gsimple_beammc_annie_${flxgrp3}*.root,${FLXPSET}"
#fi

# put it all together
# XSEC="--cross-sections $GENIEXSECFILE"   # previously defined by: setup genie_xsec
XSEC="--cross-sections $GENIEXSEC"
...
echo " "
  ...
  if [ -z "$MESTHRE" ]; then
    echo $EXE -r $RUN $SEEDFLG $TOPVOLFLG -f $FLUX -g $GDML -S $NRAYS $UNITS $XSEC --tune G18_10a_02_11a $GENFLG $MAXPL $LIMIT $FIDFLG $ZMINFLG
         $EXE -r $RUN $SEEDFLG $TOPVOLFLG -f $FLUX -g $GDML -S $NRAYS $UNITS $XSEC --tune G18_10a_02_11a $GENFLG $MAXPL $LIMIT $FIDFLG $ZMINFLG
  else
    echo $EXE -r $RUN $SEEDFLG $TOPVOLFLG -f $FLUX -g $GDML -S $NRAYS $UNITS $XSEC --tune G18_10a_02_11a $GENFLG $MAXPL $LIMIT $FIDFLG $ZMINFLG --message-thresholds $MESTHRE
         $EXE -r $RUN $SEEDFLG $TOPVOLFLG -f $FLUX -g $GDML -S $NRAYS $UNITS $XSEC --tune G18_10a_02_11a $GENFLG $MAXPL $LIMIT $FIDFLG $ZMINFLG --message-thresholds $MESTHRE
  fi

     #$EXE -r $RUN $SEEDFLG $TOPVOLFLG -f $FLUX -g $GDML $UNITS $XSEC --tune G18_10a_02_11a $GENFLG $MAXPL $LIMIT $MSGLVL $FIDFLG $ZMINFLG MAXPL > gntp.${RUN}.ghep.log 2>&1
     #$EXE -r $RUN $SEEDFLG $TOPVOLFLG -f $FLUX -g $GDML $UNITS $XSEC --tune G18_10a_02_11a $GENFLG $MAXPL $LIMIT $MSGLVL $FIDFLG $ZMINFLG MAXPL
#cat gntp.${RUN}.ghep.log

/cvmfs/larsoft.opensciencegrid.org/products/genie/v3_00_04_ub3/Linux64bit+3.10-2.17-e17-prof/bin/gntpc -i gntp.${RUN}.ghep.root -f gst

#scp gntp.${RUN}.gst.root noaheverett@172.20.10.9:/Users/noaheverett/Library/Mobile\ Documents/com~apple~CloudDocs/Documents/School/Other/Work\ Study/Dr.\ Wang\ Work\ Study/ANNIE_machine/runs
