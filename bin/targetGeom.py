### Known Issues
# - Will go outside the FV as much as OV_t
# - Should rename Outer and OuterArgon to be consistant with the 2 letter naming convention
...
G4geom_make = True
G4geom_parentPhysicalVolumeName = "t_parent_PV"
G4geom_scubaTank_material_name   = "StainlessSteel"
G4geom_outerVolume_material_name = "StainlessSteel"

GDML_make = False
GDML_name = "annie_v02_scubaTank"
GDML_targetPosition = 'o' # i=inside, m=middle, o=outside
GDML_addTargetPosition = True
GDML_STmaterial = 'ts' # s=Steel, ts=SankSteel a=Aluminum

GArD_S = 1.652 / 100**3 * 0.986923 # density of GAr at 18 C and 1 atm [kg/cm^3]
TS_D = 7.841/1000 # density of Tank Steel [kg/cm^3]

ST_d   = 22  # diameter         [cm]
ST_r   = ST_d/2
ST_h   = 75  # height           [cm]
ST_t   = 0.5 # thickness        [cm]
ST_p   = 300 # pressure         [atm]
ST_V   = np.pi * (ST_d/2-ST_t)**2 * (ST_h-2*ST_t) # volume [cm^3]

OV_p   = 5   # pressure         [atm]
OV_l   = 2   # padding on sides [cm]
OV_t   = 0.5 # thickness        [cm]

FV_x   = 100 # beam direction   [cm]
FV_y   = 200 # height           [cm]
FV_z   = 100 # transverse       [cm]

FV_L_x = 0   # x location of FV [cm]
FV_L_y = 0   # y location of FV [cm]
FV_L_z = 0   # z location of FV [cm]

nSTWide = 3 # number of tanks in primary layer
nSTHigh = 2 # number of tanks in secondary layer

### Check arguments:
GDML_targetNameMap = {'i':'inside', 'm':'middle', 'o':'outside'}
if GDML_targetPosition not in GDML_targetNameMap.keys(): raise ValueError("Invalid GDML_targetPosition")
if GDML_addTargetPosition: GDML_name = GDML_name + '_' + GDML_targetNameMap[GDML_targetPosition] + 'FV.gdml'
else                     : GDML_name = GDML_name + '.gdml'
GDML_STmaterialMap = {'s':'Steel', 'ts':'TankSteel', 'a':'Aluminum'}
if GDML_STmaterial not in GDML_STmaterialMap.keys(): raise ValueError("Invalid GDML_STmaterial")

### GAr Densities
GArD_ST = GArD_S * ST_p # density of ST GAr [kg/cm^3]
GArD_OV = GArD_S * OV_p # density of OV GAr [kg/cm^3]
print()
print("#===== GAr Densities =====#")
print("ST Density--: {0:.6f} kg/cm3".format(GArD_ST))
print("          --: {0:.6f} kg/m3" .format(GArD_ST*100**3))
print("OV Density--: {0:.6f} kg/cm3".format(GArD_OV))
print("          --: {0:.6f} kg/m3" .format(GArD_OV*100**3))
print()

### ST Stats:
ST_M = ST_V * GArD_ST # mass per tank [kg]
print("#===== Scuba Tank Stats =====#")
print("Volume--------: {0:.2f} cm3"   .format(ST_V))
print("Pressure------: {0:.2f} atm"   .format(ST_p))
print("Density-------: {0:.2f} kg/cm3".format(GArD_ST))
print("Mass per Tank-: {0:.2f} kg"    .format(ST_M))
print("Total Mass----: {0:.2f} kg"    .format(ST_M * (2*nSTWide-1)))
print()
...
### primary layer (PL):
PL_pos_z = np.linspace(-FV_x/2+ST_r+OV_l, FV_x/2-ST_r-OV_l, nSTWide)
if   nSTHigh == 1: PL_pos_y = [0]
elif nSTHigh == 2: PL_pos_y = [-ST_h/2, ST_h/2]
elif nSTHigh == 3: PL_pos_y = [-ST_h, 0, ST_h+2]
if verbose: print("#===== Primary Layer (verbose) =====#")
if verbose: print("PL_pos_z: {0}".format(PL_pos_z))
if verbose: print("PL_pos_y: {0}".format(PL_pos_y))
if verbose: print()
for z in PL_pos_z:
    for y in PL_pos_y:
        STLocations.append([0, y, z])

### seconday layer:
SLzRemainder = FV_z - 2*OV_l - nSTWide*ST_d
SLzRemaiderPerTank = SLzRemainder / (nSTWide-1)
SL_pos_z = np.linspace(-FV_x/2+ST_d+OV_l+SLzRemaiderPerTank/2, FV_x/2-ST_d-OV_l-SLzRemaiderPerTank/2, nSTWide-1)
overlap_z = ST_d/2 - SLzRemaiderPerTank/2
theta = np.arccos((ST_r-overlap_z)/ST_r)
incidentPosX = -ST_r*np.sin(theta)
SL_pos_x = 2*incidentPosX
if verbose: print("#===== Secondary Layer (verbose) =====#")
if verbose: print("SLzRemainder: {0:.2f}".format(SLzRemainder))
if verbose: print("SLzRemainderPerTank: {0:.2f}".format(SLzRemaiderPerTank))
if verbose: print("overlap_z: {0:.2f}".format(overlap_z))
if verbose: print("theta: {0:.2f} rad or {1:.2f} deg".format(theta, np.degrees(theta)))
if verbose: print("incidentPosX: {0:.2f}".format(incidentPosX))
if verbose: print("SL_pos_x: {0}".format(SL_pos_x))
if verbose: print("SL_pos_y: {0}".format(PL_pos_y))
if verbose: print("SL_pos_z: {0}".format(SL_pos_z))
if verbose: print()
# SL_pos_x = [
for z in SL_pos_z:
    for y in PL_pos_y:
        STLocations.append([SL_pos_x, y, z])

### Adjust X values:
deltaX = SL_pos_x/2
for i, loc in enumerate(STLocations):
    STLocations[i][0] -= deltaX

### ST Locations:
print("#===== Scuba Tank Locations (x,y,z) =====#")
for i, loc in enumerate(STLocations):
    # print("Tank {0}--: ({1:+06.2f}, {2:+06.2f}, {3:+06.2f})".format(i, loc[0], loc[1], loc[2]))
    print("Tank {0}--: ({1:.2f}, {2:.2f}, {3:.2f})".format(i, loc[0], loc[1], loc[2]))
print()

### OV Edges:
ST_location_limits = [
   [min([loc[0] for loc in STLocations])-ST_r   ,
    max([loc[0] for loc in STLocations])+ST_r  ],
   [min([loc[1] for loc in STLocations])-ST_h/2 , 
    max([loc[1] for loc in STLocations])+ST_h/2],
   [min([loc[2] for loc in STLocations])-ST_r   ,
    max([loc[2] for loc in STLocations])+ST_r  ]
]
if verbose: print("#===== Scuba Tank Location Limits (x,y,z) =====#")
if verbose: print("ST_location_limits[0]: {0}".format(ST_location_limits[0]))
if verbose: print("ST_location_limits[1]: {0}".format(ST_location_limits[1]))
if verbose: print("ST_location_limits[2]: {0}".format(ST_location_limits[2]))
if verbose: print()
OA_size = [
    np.diff(ST_location_limits[0])[0] + 2*OV_l,
    np.diff(ST_location_limits[1])[0] + 2*OV_l,
    np.diff(ST_location_limits[2])[0] + 2*OV_l
]
if verbose: print("#===== Outer Argon Size (x,y,z) =====#")
if verbose: print("OA_size[0]: {0}".format(OA_size[0]))
if verbose: print("OA_size[1]: {0}".format(OA_size[1]))
if verbose: print("OA_size[2]: {0}".format(OA_size[2]))
OV_size = [
    OA_size[0] + 2*OV_t,
    OA_size[1] + 2*OV_t,
    OA_size[2] + 2*OV_t
]
if verbose: print("#===== Outer Volume Size (x,y,z) =====#")
if verbose: print("OV_size[0]: {0}".format(OV_size[0]))
if verbose: print("OV_size[1]: {0}".format(OV_size[1]))
if verbose: print("OV_size[2]: {0}".format(OV_size[2])) 
if verbose: print()
OA_limits = [
    [ST_location_limits[0][0]-OV_l, ST_location_limits[0][1]+OV_l],
    [ST_location_limits[1][0]-OV_l, ST_location_limits[1][1]+OV_l],
    [ST_location_limits[2][0]-OV_l, ST_location_limits[2][1]+OV_l]
]
if verbose: print("#===== Outer Argon Limits (x,y,z) =====#")
if verbose: print("OA_limits[0]: {0}".format(OA_limits[0]))
if verbose: print("OA_limits[1]: {0}".format(OA_limits[1]))
if verbose: print("OA_limits[2]: {0}".format(OA_limits[2]))
OV_limits = [
    [OA_limits[0][0]-OV_t, OA_limits[0][1]+OV_t],
    [OA_limits[1][0]-OV_t, OA_limits[1][1]+OV_t],
    [OA_limits[2][0]-OV_t, OA_limits[2][1]+OV_t]
]
if verbose: print("#===== Outer Volume Limits (x,y,z) =====#")
if verbose: print("OV_limits[0]: {0}".format(OV_limits[0]))
if verbose: print("OV_limits[1]: {0}".format(OV_limits[1]))
if verbose: print("OV_limits[2]: {0}".format(OV_limits[2]))
if verbose: print()
OA_edges = [
    [OA_limits[0][0], OA_limits[1][0], OA_limits[2][0]],
    [OA_limits[0][0], OA_limits[1][0], OA_limits[2][1]],
    [OA_limits[0][1], OA_limits[1][0], OA_limits[2][1]],
    [OA_limits[0][1], OA_limits[1][0], OA_limits[2][0]],
    [OA_limits[0][0], OA_limits[1][1], OA_limits[2][0]],
    [OA_limits[0][0], OA_limits[1][1], OA_limits[2][1]],
    [OA_limits[0][1], OA_limits[1][1], OA_limits[2][0]],
    [OA_limits[0][1], OA_limits[1][1], OA_limits[2][1]]
]
if verbose: print("#===== Outer Argon Edges (x,y,z) =====#")
if verbose:
    for i, edge in enumerate(OA_edges):
        print("OA_edges[{0}]: {1}".format(i, edge))
OV_edges = [
    [OV_limits[0][0], OV_limits[1][0], OV_limits[2][0]],
    [OV_limits[0][0], OV_limits[1][0], OV_limits[2][1]],
    [OV_limits[0][1], OV_limits[1][0], OV_limits[2][1]],
    [OV_limits[0][1], OV_limits[1][0], OV_limits[2][0]],
    [OV_limits[0][0], OV_limits[1][1], OV_limits[2][0]],
    [OV_limits[0][0], OV_limits[1][1], OV_limits[2][1]],
    [OV_limits[0][1], OV_limits[1][1], OV_limits[2][0]],
    [OV_limits[0][1], OV_limits[1][1], OV_limits[2][1]]
]
if verbose: print("#===== Outer Volume Edges (x,y,z) =====#")
if verbose: 
    for i, edge in enumerate(OV_edges): 
        print("OV_edges[{0}]: {1}".format(i, edge))
if verbose: print()

### OA Stats:
OA_TV = (OA_limits[0][1]-OA_limits[0][0]) * (OA_limits[1][1]-OA_limits[1][0]) * (OA_limits[2][1]-OA_limits[2][0]) # Total Volume [cm^3]
OA_UV = OA_TV - sum([ST_V for _ in STLocations]) # Unused Volume [cm^3]
OA_M = OA_UV * GArD_OV # mass [kg]
print("#===== Outer Argon Stats =====#")
print("Total Volume--: {0:.2f} cm3"   .format(OA_TV))
print("Unused Volume-: {0:.2f} cm3"   .format(OA_UV))
print("Density-------: {0:.4f} kg/cm3".format(GArD_OV))
print("Mass----------: {0:.4f} kg"    .format(OA_M))

### OV Stats:
OV_TV = (OV_limits[0][1]-OV_limits[0][0]) * (OV_limits[1][1]-OV_limits[1][0]) * (OV_limits[2][1]-OV_limits[2][0]) # Total Volume [cm^3]
OV_UV = OV_TV - OA_TV # Unused Volume [cm^3]
OV_M = OV_UV * TS_D # mass [kg]
print("#===== Outer Volume Stats =====#")
print("Total Volume--: {0:.2f} cm3"   .format(OV_TV))
print("Unused Volume-: {0:.2f} cm3"   .format(OV_UV))
print("Density-------: {0:.4f} kg/cm3".format(TS_D))
print("Mass----------: {0:.4f} kg"    .format(OV_M))
print()

### Overall Tank Stats:
print("#===== Overall Tank Stats =====#")
print("Total Argon Mass----: {0:.2f} kg"    .format(ST_M * (2*nSTWide-1) + OA_M))
print()

### Prepare for plotting:
np.random.shuffle(STLocations)
cmap = plt.get_cmap('rainbow')
colors = [cmap(i) for i in np.linspace(0, 1, len(STLocations))]
if verbose: print("#===== Colors =====#")
if verbose: print("colors: {0}".format(colors))
if verbose: print()
...
if plot_2d:
    ...
    ax[0].set_xlim(-FV_z/2-plot_padding, FV_z/2+plot_padding)
    ax[0].set_ylim(-FV_x/2-plot_padding, FV_x/2+plot_padding)
    ax[0].set_aspect('equal')
    for i, loc in enumerate(STLocations):
        ax[0].add_patch(plt.Circle((loc[2], loc[0]), ST_r, color=colors[i%len(colors)], alpha=0.5))
        ax[0].add_patch(plt.Circle((loc[2], loc[0]), ST_r-ST_t, color='k', alpha=0.2))
    ax[0].plot(
        [OV_edges[i][2] for i in [0, 1, 2, 3, 0]],
        [OV_edges[i][0] for i in [0, 1, 2, 3, 0]],
        'k-'
    )
    ax[0].plot(
        [OA_edges[i][2] for i in [0, 1, 2, 3, 0]],
        [OA_edges[i][0] for i in [0, 1, 2, 3, 0]],
        'k--'
    )
    ...
    ax[1].set_xlim(-FV_x/2-plot_padding, FV_x/2+plot_padding)
    ax[1].set_ylim(-FV_y/2-plot_padding, FV_y/2+plot_padding)
    ax[1].set_aspect('equal')
    for i, loc in enumerate(STLocations):
        ax[1].add_patch(plt.Rectangle((loc[0]-ST_r, loc[1]-ST_h/2), ST_d, ST_h, color=colors[i%len(colors)], alpha=0.5))
        ax[1].add_patch(plt.Rectangle((loc[0]-ST_r+ST_t, loc[1]-ST_h/2+ST_t), ST_d-2*ST_t, ST_h-2*ST_t, color='k', alpha=0.2))
    ax[1].plot(
        [OV_edges[i][0] for i in [0, 4, 6, 2, 0]],
        [OV_edges[i][1] for i in [0, 4, 6, 2, 0]],
        'k-'
    )
    ax[1].plot(
        [OA_edges[i][0] for i in [0, 4, 6, 2, 0]],
        [OA_edges[i][1] for i in [0, 4, 6, 2, 0]],
        'k--'
    )
    ...
    ax[2].set_xlim(-FV_z/2-plot_padding, FV_z/2+plot_padding)
    ax[2].set_ylim(-FV_y/2-plot_padding, FV_y/2+plot_padding)
    ax[2].set_aspect('equal')
    for i, loc in enumerate(STLocations):
        ax[2].add_patch(plt.Rectangle((loc[2]-ST_r, loc[1]-ST_h/2), ST_d, ST_h, color=colors[i%len(colors)], alpha=0.5))
        ax[2].add_patch(plt.Rectangle((loc[2]-ST_r+ST_t, loc[1]-ST_h/2+ST_t), ST_d-2*ST_t, ST_h-2*ST_t, color='k', alpha=0.2))
    ax[2].plot(
        [OV_edges[i][2] for i in [0, 1, 5, 4, 0]],
        [OV_edges[i][1] for i in [0, 1, 5, 4, 0]],
        'k-'
    )
    ax[2].plot(
        [OA_edges[i][2] for i in [0, 1, 5, 4, 0]],
        [OA_edges[i][1] for i in [0, 1, 5, 4, 0]],
        'k--'
    )
...
if plot_3d:
    ...
    ax.set_ylabel("x [cm]")
    ax.set_zlabel("y [cm]")
    ax.set_xlim(-FV_z/2, FV_z/2)
    ax.set_ylim(-FV_x/2, FV_x/2)
    ax.set_zlim(-FV_y/2, FV_y/2)

    def get_cylinder(x, y, z, r, h, n=100):
        theta = np.linspace(0, 2 * np.pi, n)
        z_vals = np.linspace(z, z + h, n)
    ...
    for i, loc in enumerate(STLocations):
        ax.plot_surface(*get_cylinder(loc[2], loc[0], loc[1]-ST_h/2, ST_r, ST_h), rstride=10, cstride=10, color=colors[i%len(colors)], alpha=0.5)
        ax.plot_surface(*get_cylinder(loc[2], loc[0], loc[1]-ST_h/2, ST_r-ST_t, ST_h-2*ST_t), rstride=10, cstride=10, color='k', alpha=0.5)

    OA_edges = np.array([OA_edges[i] for i in [0, 3, 6, 4, 1, 2, 7, 5]])
    OA_edges = np.array([OA_edges[:,2], OA_edges[:,0], OA_edges[:,1]]).T
    faces_OA = [
        [OA_edges[i] for i in [0, 1, 5, 4]],
        [OA_edges[i] for i in [7, 6, 2, 3]],
        [OA_edges[i] for i in [0, 3, 7, 4]],
        [OA_edges[i] for i in [1, 2, 6, 5]],
        [OA_edges[i] for i in [0, 1, 2, 3]],
        [OA_edges[i] for i in [4, 5, 6, 7]]
    ]
    ...
    OV_edges = np.array([OV_edges[i] for i in [0, 3, 6, 4, 1, 2, 7, 5]])
    OV_edges = np.array([OV_edges[:,2], OV_edges[:,0], OV_edges[:,1]]).T
    faces_OV = [
        [OV_edges[i] for i in [0, 1, 5, 4]],
        [OV_edges[i] for i in [7, 6, 2, 3]],
        [OV_edges[i] for i in [0, 3, 7, 4]],
        [OV_edges[i] for i in [1, 2, 6, 5]],
        [OV_edges[i] for i in [0, 1, 2, 3]],
        [OV_edges[i] for i in [4, 5, 6, 7]]
    ]
    ax.add_collection3d(Poly3DCollection(
        faces_OV,
        facecolors='red', 
...
//   |---> `{G4geom_parentPhysicalVolumeName}`: the name of the parent physical volume
//   |---> `{G4geom_scubaTank_material_name}`: the material of the scuba tank
//   |---> `{G4geom_outerVolume_material_name}`: the material name of the outer volume

// Variables:
G4double ST_r = {ST_r}*cm; // radius of the scuba tank [cm]
G4double ST_h = {ST_h}*cm; // height of the scuba tank [cm]
G4double ST_t = {ST_t}*cm; // thickness of the scuba tank [cm]
G4double OA_size[3] = {{ {OA_size[2]}*cm, {OA_size[0]}*cm, {OA_size[1]}*cm }}; // size of the outer argon [cm]
G4double OV_size[3] = {{ {OV_size[2]}*cm, {OV_size[0]}*cm, {OV_size[1]}*cm }}; // size of the outer volume [cm]
G4double STLocations[{len(STLocations)}][3] = {{
"""
    for i, loc in enumerate(STLocations):
        G4geom_str += f"    {{ {loc[2]}*cm, {loc[0]}*cm, {loc[1]}*cm }}"
        if i+1 < len(STLocations): G4geom_str += ","
        G4geom_str += "\n"
    G4geom_str += f"""\
}}; // locations of the scuba tanks from targetGeom.py [cm]
"""
    if GDML_targetPosition == 'o':
        G4geom_str += f"""\
G4ThreeVector OV_PV_pos = G4ThreeVector(0*cm, {(1519.24-600)/10-OV_size[0]/2}*cm, 0*cm);
"""
    elif GDML_targetPosition == 'i':
        G4geom_str += f"""\
G4ThreeVector OV_PV_pos = G4ThreeVector(0*cm, {(1519.24-1600)/10+OV_size[0]/2}*cm, 0*cm);
"""
    elif GDML_targetPosition == 'm':
        G4geom_str += f"""\
G4ThreeVector OV_PV_pos = G4ThreeVector(0*cm, {(1519.24-(1600+600)/2)/10}*cm, 0*cm);
"""
    G4geom_str += f"""
// Inner Argon:
G4Material* IA_M = new G4Material("IA_M", 18, 39.948*g/mole, {GArD_ST}*kg/cm3, kStateGas, 293.15*kelvin, {ST_p}*atmosphere);
...
G4LogicalVolume* ST_LV = new G4LogicalVolume(ST_S, G4Material::GetMaterial("{G4geom_scubaTank_material_name}"), "ST_LV");

// Outer Argon:
G4Material* OA_M = new G4Material("OA_M", 18, 39.948*g/mole, {GArD_OV}*kg/cm3, kStateGas, 293.15*kelvin, {OV_p}*atmosphere);
// Skipping optical properties for now
...
G4LogicalVolume* OV_LV = new G4LogicalVolume(OV_S, G4Material::GetMaterial("{G4geom_outerVolume_material_name}"), "OV_LV");

// Place outer volume:
G4PVPlacement* OV_PV = new G4PVPlacement(0, OV_PV_pos, "OV_PV", OV_LV, {G4geom_parentPhysicalVolumeName}, false, 0, true);

// Place outer argon:
G4PVPlacement* OA_PV = new G4PVPlacement(0, G4ThreeVector(0, 0, 0), "OA_PV", OA_LV, OV_PV, false, 0, true);

// Place scuba tanks:
for (int i = 0; i < {len(STLocations)}; i++) {{
...
"""
    ...
    print("#===== G4 Geometry -- BEGIN =====#")
    print(G4geom_str)
    print("#===== G4 Geometry -- END =====#")
    print()

if GDML_make:
    G4_IA = f"""\
    <material name="G4_IA" state="gas">
      <T unit="K" value="293.15"/>
      <MEE unit="eV" value="188.000"/>
      <D unit="g/cm3" value="{GArD_ST*1000}"/>
      <fraction n="1" ref="Ar"/>
    </material>\
"""
    G4_OA = f"""\
    <material name="G4_OA" state="gas">
        <T unit="K" value="293.15"/>
        <MEE unit="eV" value="188.000"/>
        <D unit="g/cm3" value="{GArD_OV*1000}"/>
        <fraction n="1" ref="Ar"/>
    </material>\
"""
#     Ar_LV = ""
...
#     for i, loc in enumerate(STLocations):
#         Ar_LV += f"""\n
# <tube aunit=\"deg\" deltaphi=\"360\" lunit=\"cm\" name=\"TARGON_{i}_S\" rmax=\"{ST_r-ST_t}\" rmin=\"0\" startphi=\"0\" z=\"{ST_h-2*ST_t}\"/>
# """
#         ST_LV += f"""
# <tube aunit="deg" deltaphi="360" lunit="cm" name="TSCUBATANK_{i}_S" rmax="{ST_r}" rmin="{0}" startphi="0" z="{ST_h}"/>
# """
    IA_S = f"""\
    <tube aunit="deg" deltaphi="360" lunit="cm" name="TINNERARGON_S" rmax="{ST_r-ST_t}" rmin="0" startphi="0" z="{ST_h-2*ST_t}"/>\
"""
    ST_S = f"""\
    <tube aunit="deg" deltaphi="360" lunit="cm" name="TSCUBATANK_S" rmax="{ST_r}" rmin="{0}" startphi="0" z="{ST_h}"/>\
"""
    OA_S = f"""\
    <box lunit="cm" name="TOUTERARGON_S" x="{OA_size[2]}" y="{OA_size[0]}" z="{OA_size[1]}"/>\
"""
    OV_S = f"""\
    <box lunit="cm" name="TOUTERVOLUME_S" x="{OV_size[2]}" y="{OV_size[0]}" z="{OV_size[1]}"/>\
"""
    if GDML_targetPosition == 'o':
        OV_PV_pos = f"<position name=\"TOUTERVOLUME_PV_pos\" unit=\"cm\" x=\"{0}\" y=\"{(1519.24-600)/10-OV_size[0]/2}\" z=\"{0}\"/>"
    elif GDML_targetPosition == 'i':
        OV_PV_pos = f"<position name=\"TOUTERVOLUME_PV_pos\" unit=\"cm\" x=\"{0}\" y=\"{(1519.24-1600)/10+OV_size[0]/2}\" z=\"{0}\"/>"
    elif GDML_targetPosition == 'm':
        OV_PV_pos = f"<position name=\"TOUTERVOLUME_PV_pos\" unit=\"cm\" x=\"{0}\" y=\"{(1519.24-(1600+600)/2)/10}\" z=\"{0}\"/>"
    Water_LV = f"""\
    <volume name="TWATER_LV">
      <materialref ref="TankWater"/>
      <solidref ref="TWATER_S"/>
      <physvol name="TOUTERVOLUME_PV">
        <volumeref ref="TOUTERVOLUME_LV"/>
        {OV_PV_pos}
      </physvol>
    </volume>\
"""
    ...
    for i, loc in enumerate(STLocations):
        ST_LV_ref += f"""\
        <physvol name="TSCUBATANK_{i}_PV">
          <volumeref ref="TSCUBATANK_{i}_LV"/>
          <position name="TSCUBATANK_{i}_PV_pos" unit="cm" x="{loc[2]}" y="{loc[0]}" z="{loc[1]}"/>
        </physvol>\
"""
        if i+1 < len(STLocations):
            ST_LV_ref += "\n"

    OA_LV = f"""\
    <volume name="TOUTERARGON_LV">
      <materialref ref="G4_OA"/>
      <solidref ref="TOUTERARGON_S"/>
""" + ST_LV_ref + """
    </volume>\
"""

    OV_LV = f"""\
    <volume name="TOUTERVOLUME_LV">
      <materialref ref="TankSteel"/>
      <solidref ref="TOUTERVOLUME_S"/>
      <physvol name="TOUTERARGON_PV">
        <volumeref ref="TOUTERARGON_LV"/>
        <position name="TOUTERARGON_PV_pos" unit="cm" x="{0}" y="{0}" z="{0}"/>
      </physvol>
    </volume>\
"""
    ST_LV = ""
    for i, loc in enumerate(STLocations):
        ST_LV += f"""\
    <volume name="TSCUBATANK_{i}_LV">
      <materialref ref="{GDML_STmaterialMap[GDML_STmaterial]}"/>
      <solidref ref="TSCUBATANK_S"/>
      <physvol name="TINNERARGON_{i}_PV">
        <volumeref ref="TINNERARGON_{i}_LV"/>
        <position name="TINNERARGON_{i}_PV_pos" unit="cm" x="{0}" y="{0}" z="{0}"/>
      </physvol>
    </volume>\
"""
        if i+1 < len(STLocations):
    ...
    for i, loc in enumerate(STLocations):
        IA_LV += f"""\
    <volume name="TINNERARGON_{i}_LV">
      <materialref ref="G4_IA"/>
      <solidref ref="TINNERARGON_S"/>
    </volume>\
"""
        if i+1 < len(STLocations):
            IA_LV += "\n"

    with open(GDML_name, "w") as file:
